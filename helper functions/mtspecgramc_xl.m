function [S,t,f,Serr]=mtspecgramc_xl(data,movingwin,params)
...
if trialave
    S = zeros(nw,Nf,tapers(2));  % Add additional dimension tapers. X.L.
    if nargout==4; Serr=zeros(2,nw,Nf); end;
else
    S = zeros(nw,Nf,Ch,tapers(2)); % Add additional dimension tapers. X.L.
...
for n=1:nw;
   ...
   datawin=data(indx,:);
   if nargout==4
     [s,f,serr]=mtspectrumc_XL(datawin,params);
     Serr(1,n,:,:)=squeeze(serr(1,:,:));
     Serr(2,n,:,:)=squeeze(serr(2,:,:));
   else
     [s,f]=mtspectrumc_XL(datawin,params);
   end
   S(n,:,:,:)=s;
end;
S=squeeze(S); 
