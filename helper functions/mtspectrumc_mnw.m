function [S,phi,allphi,f,Serr]=mtspectrumc_mnw(data,params)
...
J=J(findx,:,:);
S=[1,2,3];%S=permute(mean(conj(J).*J,2),[1 3 2]);
r = sum(exp(1i*angle(J)),2);
phi=permute(angle(r),[1 3 2]);
allphi = permute(angle(J), [1,3,2]);
if trialave
    %S=squeeze(mean(S,2));
    r2 = sum(exp(1i*angle(phi)),2);
    phi=squeeze(angle(r2));
else
    %S=squeeze(S);
    phi=squeeze(phi);
end;
