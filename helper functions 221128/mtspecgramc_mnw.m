function [S,phi,allphiA,t,f,Serr]=mtspecgramc_mnw(data,movingwin,params)
% Chronux function modified by Maddie Wilson
%
...
if trialave
    S = zeros(nw,Nf);
    phi = zeros(nw,Nf);
    allphiA = zeros(nw,Nf,nTapers);
    if nargout==6; Serr=zeros(2,nw,Nf); end;
else
    S = zeros(nw,Nf,Ch);
    phi = zeros(nw,Nf,Ch);
    allphiA = zeros(nw,Nf,Ch,nTapers);
    if nargout==6; Serr=zeros(2,nw,Nf,Ch); end;
end
...
for n=1:nw;
   ...
   datawin=data(indx,:);
   if nargout==6
     [s,p,allphi,f,serr]=mtspectrumc_mnw(datawin,params);
     Serr(1,n,:,:)=squeeze(serr(1,:,:));
     Serr(2,n,:,:)=squeeze(serr(2,:,:));
   else
     [s,p,allphi,f]=mtspectrumc_mnw(datawin,params);
   end
   %S(n,:,:)=s;
   phi(n,:,:)=p;
   allphiA(n,:,:,:) = allphi;
...
%S=squeeze(S); 
phi=squeeze(phi);
if nargout==6;Serr=squeeze(Serr);end;
